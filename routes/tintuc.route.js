...
var TinTucModel = require('../model/tintuc.model');
...
router.get('/:url', function(req, res, next) {
  async.auto({
    linkbar:function(done) {
        ...
        linkbar[1] = linkbar[0] + "/tin-tuc/" + req.params.url;
        var url_title = '';
          ...
          if( data.getData ) {
            ...
            for(var i = 0; i < length; i++) {
              if(i === 0) {
                html_head = html_head
                        + '<div class="col-md-6">'
                        +    '<a href="'+ config.domain + "/tin-tuc/" + req.params.url +"/" + data.getData[i].url +'">'
                        ...
                        +             '<a href="'+ config.domain + "/tin-tuc/"+ req.params.url +"/" + data.getData[i].url +'"><h3 class="no-margin-top">'+data.getData[i].title+'</h3></a>'
                        +         '</h3>'
                        +     '</header>'
                        +     '<div>'
                ...
                +     '<div class="col-sm-2">'
                +       '<div>'
                +          '<a href="'+ config.domain + "/"+ req.params.url +"/tin-tuc/"  + data.getData[i].url +'" class="full-image">'
                ...
                +             '<a href="'+ config.domain + "/"+ req.params.url +"/tin-tuc/"  + data.getData[i].url +'"><h3 class="no-padding-top no-margin-top">'+ data.getData[i].title +'</h3></a>'
                +         '</h4>'
    ...
    social: ['content', function(data, done) {
      ...
      social = social
        + '<div class="social">'
        +	  '<span class="Facebook">'
        +    '<div class="fb-like" data-href="' + config.domain + '/tin-tuc/' + req.params.url +'" data-layout="button_count" data-action="like" data-size="small" data-show-faces="true" data-share="true"></div>'
        +	'</span>'
        + '<span class="google">'
...
router.get('/:type/:title_url', function(req, res, next) {
  async.auto({
    data: function(done) {
      ...
      TinTucModel.findOne(query).lean().exec(done);
    },
    linkbar: ['data', function(data, done) {
      if(data.data) {
        ...
        linkbar[0] = config.domain;
        linkbar[1] = linkbar[0] + "/tin-tuc/" + req.params.type;
    ...
    getDataFooter: function(done) {
      TinTucModel.aggregate([ { $project : { _id: 0, "url":1, "image":1, "type":1, "url":1, "title":1, "description": 1} }
              ]).exec(done);
    },
          ...
          if( data.getDataFooter) {
            ...
            for(var i = 0; i < length; i++) {
              html = html
                ...
                +     '<div class="col-sm-2">'
                +       '<div>'
                +          '<a href="'+ config.domain + "/tin-tuc/"+ data.getDataFooter[i].type + "/"+ data.getDataFooter[i].url + '" class="full-image">'
                ...
                +     '<div class="col-sm-10">'
                +         '<h4 class="no-padding-top no-margin-top">'
                +             '<a href="'+ config.domain + "/tin-tuc/"+ data.getDataFooter[i].type + "/"+ data.getDataFooter[i].url + '">'+ data.getDataFooter[i].title +'</a>'
                +         '</h4>'
                +         '<div class="col-md-12 no-padding-left">'
                +             '<div class="entry-content">'
    ...
    footer2: ['getDataFooter', function(data, done) {
      if(data) {
          ...
          if( data.getDataFooter) {
            ...
            for(var i = 0; i < length; i++) {
              html = html
                      + '<li><a href="'+ config.domain + "/tin-tuc/"+ data.getDataFooter[i].type + "/"+ data.getDataFooter[i].url + '">'+ data.getDataFooter[i].title +'</a></li>'
            }
          }
    ...
    social: ['footer2', function(data, done) {
      ...
      social = social
        + '<div class="social">'
        +	  '<span class="Facebook">'
        +    '<div class="fb-like" data-href="' + config.domain + '/tin-tuc/' + req.params.type + '/' + req.params.title_url +'" data-layout="button_count" data-action="like" data-size="small" data-show-faces="true" data-share="true"></div>'
        +	'</span>'
        + '<span class="google">'
        +   '<div class="g-plusone" data-size="medium" data-annotation="inline" data-width="300"></div>'
        +	'</span>'
  ...
  function(err, data) {
    ...
    if(data.data) {
      ...
      res.render('TinTuc/tintucdetail', {
        title:  req.url_title + "- Sửa chữa cải tạo Hưng Thịnh",
        body: body,
        linkbar: data.linkbar,
        footer: data.footer,
